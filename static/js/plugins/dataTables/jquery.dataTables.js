...
(/** @lends <global> */function( $ ) {
	...
	function _fnBrowserDetect( settings )
	{
		...
		// In rtl text base, some browsers (most, but not all) will place the
	 ...
	 * methodology here is to use the base array from _fnDetectHeader, modified for
	 * the instantaneous column visibility, to construct the new base. The grid is
	...
	function _fnDrawHead( oSettings, aoSource, bIncludeHidden )
	{
		...
		/* Make a copy of the master base array, but without the visible columns in it */
	 ...
	 * create a base grid (array) of rows x columns, which contains a reference
	 * to the cell that that point in the grid (regardless of col/rowspan), such that
	 * any column / row could be removed and the new grid constructed
	 *  @param array {object} aLayout Array to store the calculated base in
	...
	function _fnDetectHeader ( aLayout, nThead )
	{
		...
		/* We know how many rows there are in the base - so prep it */
		for ( i=0, iLen=nTrs.length ; i<iLen ; i++ )
		{
			aLayout.push( [] );
		}

		/* Calculate a base array */
		for ( i=0, iLen=nTrs.length ; i<iLen ; i++ )
		{
			nTr = nTrs[i];
			iColumn = 0;
			...
			while ( nCell ) {
				     ...
				     nCell.nodeName.toUpperCase() == "TH" )
				{
					...
					/* If there is col / rowspan, copy the information into the base grid */
	 ...
	 *  @param {object} oSettings dataTables settings object
	 *  @param {node} nHeader automatically detect the base from this node - optional
	 *  @param {array} aLayout thead/tfoot base from _fnDetectHeader - optional
	...
	DataTable = function( options )
	{
		...
		this.each(function() {
			...
			if ( oInit.bJQueryUI )
			{
				...
				{
					/* Set the DOM to use a base suitable for jQuery UI's theming */
	...
	DataTable.defaults = {
		 ...
		 * the table's viewport to the given height at all times (useful for base).
		 ...
		 * certain base, or you have a large number of columns in the table, you
	...
	DataTable.models.oSettings = {
		...
		"oBrowser": {
			 ...
			 * scrolling container - needed for rtl language base, although not
